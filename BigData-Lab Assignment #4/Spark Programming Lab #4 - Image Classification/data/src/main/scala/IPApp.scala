/**
  ...
  * Assignment #4 - Image Classification + Decision Tree
  * Implement K-Mean clustering for the clusters of chimpanzee's activities.
  * User defined dataset. --- traindata ("bubbleTea", "chocolate", "coffee", "mochaTea", "shavedIce")
  * #20 Chia-Hui Amy Lin
  * =====================================================================
  */
...
object IPApp {
  ...
  val IMAGE_CATEGORIES = List("bubbleTea", "chocolate", "coffee", "mochaTea")
  ...
  //  Create histogram for all the images
  def createHistogram(sc: SparkContext, images: RDD[(String, String)]): Unit = {
    ...
    val data = images.map {
      case (name, contents) => {
        // Build vocabularies for the images
      ...
      strategies.foreach(featureSubsetStrategy => {
        impurities.foreach(impurity => {
          maxDepths.foreach(maxDepth => {
            ...
            // Use the test data to get the best accuray for the model
  ...
  def main(args: Array[String]) {
    ...
    // Print out the % of recognizing the images
